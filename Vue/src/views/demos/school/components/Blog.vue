      ...
      <b-row class="g-4 justify-content-between">
        <b-col md="5">
          <b-card no-body class="bg-transparent">
            <img :src="event18" class="card-img" alt="">
        ...
        <b-col md="7">
          <template v-for="(item, idx) in blogList" :key="idx">
            <hr class="my-4" v-if="idx">
            <b-card no-body class="bg-transparent">
              <b-row class="align-items-center">
                <b-col sm="4" lg="3">
                  <div class="bg-dark text-center p-3 rounded-2 mb-2 mb-sm-0">
                    <h2 class="text-white">{{ item.date }}</h2>
                    <span class="text-white">{{ item.month }}</span>
                  </div>
                </b-col>
                <b-col sm="8" lg="9">
                  <b-card-body class="p-0">
                    <b-card-title tag="h5"><a href="#" class="stretched-link">{{ item.title }}</a></b-card-title>
...
import { blogList } from '@/views/demos/school/components/data';
